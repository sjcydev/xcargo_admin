...
export async function createInvoice(info: Factura, factura_id: number, cliente: Cliente) {
	let trackings = info.trackings.map(({ numero_tracking, peso, base, precio }) => [
		numero_tracking,
		String(peso),
	...
	autoTable(doc, {
		body: [['Entregado por:___________________', 'Recibido por:___________________']],
		theme: 'plain',
		pageBreak: 'avoid'
	});
